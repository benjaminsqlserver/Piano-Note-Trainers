...
const chordKeys = DominantSeventhSharpNineChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = DominantSeventhSharpNineChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = DominantSeventhSharpNineChordService.buildChord(key, LEARN_OCTAVE);
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = DominantSeventhSharpNineChordService.circleOfFourths.map((entry, i) => {
    const chord = DominantSeventhSharpNineChordService.buildChord(entry.key, octave, true);
...
async function playCircleOfFourths() {
  ...
  const sequence = DominantSeventhSharpNineChordService.circleOfFourths;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.dominantSeventhSharpNineJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.dominantSeventhSharpNineGospelProgressions, midi);
