...
const chordKeys = DiminishedMajorSeventhChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = DiminishedMajorSeventhChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} °(maj7)`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = DiminishedMajorSeventhChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}°(maj7)</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = DiminishedMajorSeventhChordService.circleOfFourths.map((entry, i) => {
    const chord = DiminishedMajorSeventhChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}°(maj7)</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = DiminishedMajorSeventhChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}°(maj7)`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.diminishedMajorSeventhJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.diminishedMajorSeventhGospelProgressions, midi);
