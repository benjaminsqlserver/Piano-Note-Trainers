...
const chordKeys = DominantThirteenthChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = DominantThirteenthChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
  learnKeyboard.update({ activeNote: null, activeNotes: [], tonicPitchClass: key.semitoneFromC });
  lc.stepLabel.textContent = 'Step 1 of 7';
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} 13`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = DominantThirteenthChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}13</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td><td>${chord[6].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = DominantThirteenthChordService.circleOfFourths.map((entry, i) => {
    const chord = DominantThirteenthChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}13</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td><td>${chord[6].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = DominantThirteenthChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}13`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.dominantThirteenthJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.dominantThirteenthGospelProgressions, midi);
