...
const chordKeys = DiminishedChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnTriad() {
  ...
  learnTriad = DiminishedChordService.buildTriad(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} diminished`;
...
function renderAllTriadsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const triad = DiminishedChordService.buildTriad(key, LEARN_OCTAVE);
    return `<tr><td>${key.name} diminished</td><td>${triad[0].noteName}</td><td>${triad[1].noteName}</td><td>${triad[2].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = DiminishedChordService.circleOfFourths.map((entry, i) => {
    const triad = DiminishedChordService.buildTriad(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name} diminished</td><td>${triad[0].noteName}</td><td>${triad[1].noteName}</td><td>${triad[2].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = DiminishedChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name} diminished`;
...
cp.description.textContent = DiminishedChordService.progression.description;
...
function renderProgressionTable(key, octave) {
  const chords = DiminishedChordService.buildProgression(key, octave);
