...
const chordKeys = MinorThirteenthChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MinorThirteenthChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} m13`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MinorThirteenthChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}m13</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td><td>${chord[6].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MinorThirteenthChordService.circleOfFourths.map((entry, i) => {
    const chord = MinorThirteenthChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}m13</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td><td>${chord[6].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = MinorThirteenthChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}m13`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.minorThirteenthJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.minorThirteenthGospelProgressions, midi);
