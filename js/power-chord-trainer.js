...
const chordKeys = PowerChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = PowerChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
  learnKeyboard.update({ activeNote: null, activeNotes: [], tonicPitchClass: key.semitoneFromC });
  lc.stepLabel.textContent = 'Step 1 of 2';
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} 5`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = PowerChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}5</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = PowerChordService.circleOfFourths.map((entry, i) => {
    const chord = PowerChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}5</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = PowerChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}5`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.powerChordJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.powerChordGospelProgressions, midi);
