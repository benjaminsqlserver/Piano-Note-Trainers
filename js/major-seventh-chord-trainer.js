...
const chordKeys = MajorSeventhChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MajorSeventhChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} major 7th`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MajorSeventhChordService.buildChord(key, LEARN_OCTAVE);
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MajorSeventhChordService.circleOfFourths.map((entry, i) => {
    const chord = MajorSeventhChordService.buildChord(entry.key, octave, true);
...
async function playCircleOfFourths() {
  ...
  const sequence = MajorSeventhChordService.circleOfFourths;
...
cp.description.textContent = MajorSeventhChordService.progression.description;
...
function renderProgressionTable(key, octave) {
  const chords = MajorSeventhChordService.buildProgression(key, octave);
