// minor-eleventh-chord-trainer.js — page logic for minor-eleventh-chord-trainer.html
...
const chordKeys = MinorEleventhChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MinorEleventhChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} m11`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MinorEleventhChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}m11</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MinorEleventhChordService.circleOfFourths.map((entry, i) => {
    const chord = MinorEleventhChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}m11</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td><td>${chord[5].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = MinorEleventhChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}m11`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.minorEleventhJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.minorEleventhGospelProgressions, midi);
