...
const chordKeys = AugmentedSeventhChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = AugmentedSeventhChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} augmented 7th (7\u266f5)`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = AugmentedSeventhChordService.buildChord(key, LEARN_OCTAVE);
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = AugmentedSeventhChordService.circleOfFourths.map((entry, i) => {
    const chord = AugmentedSeventhChordService.buildChord(entry.key, octave, true);
...
async function playCircleOfFourths() {
  ...
  const sequence = AugmentedSeventhChordService.circleOfFourths;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.augmentedSeventhJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.augmentedSeventhGospelProgressions, midi);
