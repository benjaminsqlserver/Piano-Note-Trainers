// dominant-seventh-flat-nine-chord-trainer.js — page logic for dominant-seventh-flat-nine-chord-trainer.html
...
const chordKeys = DominantSeventhFlatNineChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = DominantSeventhFlatNineChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} 7♭9`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = DominantSeventhFlatNineChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}7♭9</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = DominantSeventhFlatNineChordService.circleOfFourths.map((entry, i) => {
    const chord = DominantSeventhFlatNineChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}7♭9</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = DominantSeventhFlatNineChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}7♭9`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.dominantSeventhFlatNineJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.dominantSeventhFlatNineGospelProgressions, midi);
