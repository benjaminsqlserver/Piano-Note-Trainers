// major-seventh-sharp-five-chord-trainer.js — page logic for major-seventh-sharp-five-chord-trainer.html
...
const chordKeys = MajorSeventhSharpFiveChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MajorSeventhSharpFiveChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} maj7♯5`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MajorSeventhSharpFiveChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}maj7♯5</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MajorSeventhSharpFiveChordService.circleOfFourths.map((entry, i) => {
    const chord = MajorSeventhSharpFiveChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}maj7♯5</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = MajorSeventhSharpFiveChordService.circleOfFourths;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.majorSeventhSharpFiveJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.majorSeventhSharpFiveGospelProgressions, midi);
