...
const chordKeys = MinorChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnTriad() {
  ...
  learnTriad = MinorChordService.buildTriad(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} minor`;
...
function renderAllTriadsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const triad = MinorChordService.buildTriad(key, LEARN_OCTAVE);
    return `<tr><td>${key.name} major</td><td>${triad[0].noteName}</td><td>${triad[1].noteName}</td><td>${triad[2].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MinorChordService.circleOfFourths.map((entry, i) => {
    const triad = MinorChordService.buildTriad(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name} major</td><td>${triad[0].noteName}</td><td>${triad[1].noteName}</td><td>${triad[2].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = MinorChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name} minor`;
...
cp.description.textContent = MinorChordService.progression.description;
...
function renderProgressionTable(key, octave) {
  const chords = MinorChordService.buildProgression(key, octave);
