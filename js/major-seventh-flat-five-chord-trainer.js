// major-seventh-flat-five-chord-trainer.js — page logic for
// major-seventh-flat-five-chord-trainer.html
...
const chordKeys = MajorSeventhFlatFiveChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MajorSeventhFlatFiveChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} major 7th ♭5`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MajorSeventhFlatFiveChordService.buildChord(key, LEARN_OCTAVE);
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MajorSeventhFlatFiveChordService.circleOfFourths.map((entry, i) => {
...
const jazzExercise = setupProgressionPicker('jz', InversionService.majorSeventhFlatFiveJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.majorSeventhFlatFiveGospelProgressions, midi);
