...
const chordKeys = MajorChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnTriad() {
  ...
  learnTriad = MajorChordService.buildTriad(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} major`;
...
function renderAllTriadsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const triad = MajorChordService.buildTriad(key, LEARN_OCTAVE);
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MajorChordService.circleOfFourths.map((entry, i) => {
    const triad = MajorChordService.buildTriad(entry.key, octave, true);
...
async function playCircleOfFourths() {
  ...
  const sequence = MajorChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name} major`;
...
cp.description.textContent = MajorChordService.progression.description;
...
function renderProgressionTable(key, octave) {
  const chords = MajorChordService.buildProgression(key, octave);
