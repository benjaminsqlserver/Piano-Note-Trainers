...
const chordKeys = SplitThirdBluesChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = SplitThirdBluesChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} 7(♭3/♮3)`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = SplitThirdBluesChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}7(♭3/♮3)</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = SplitThirdBluesChordService.circleOfFourths.map((entry, i) => {
    const chord = SplitThirdBluesChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}7(♭3/♮3)</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = SplitThirdBluesChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}7(♭3/♮3)`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.splitThirdBluesJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.splitThirdBluesGospelProgressions, midi);
