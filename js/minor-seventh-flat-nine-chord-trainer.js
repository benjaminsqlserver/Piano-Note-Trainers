...
const chordKeys = MinorSeventhFlatNineChordService.keys; // 12 keys, chromatic order, sharp spellings
...
function rebuildLearnChord() {
  ...
  learnChord = MinorSeventhFlatNineChordService.buildChord(key, LEARN_OCTAVE);
  learnStepIndex = 0;
...
function playLearnFullChord() {
  ...
  const key = chordKeys[Number(lc.root.value)];
  lc.currentNote.textContent = `${key.name} m7♭9`;
...
function renderAllChordsTable() {
  lc.allTableBody.innerHTML = chordKeys.map((key) => {
    const chord = MinorSeventhFlatNineChordService.buildChord(key, LEARN_OCTAVE);
    return `<tr><td>${key.name}m7♭9</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td></tr>`;
  }).join('');
}
...
function renderCircleOfFourthsTable() {
  ...
  cf.tableBody.innerHTML = MinorSeventhFlatNineChordService.circleOfFourths.map((entry, i) => {
    const chord = MinorSeventhFlatNineChordService.buildChord(entry.key, octave, true);
    return `<tr data-position="${i}">
      <td>${entry.position}</td><td>${entry.name}m7♭9</td><td>${chord[0].noteName}</td><td>${chord[1].noteName}</td><td>${chord[2].noteName}</td><td>${chord[3].noteName}</td><td>${chord[4].noteName}</td>
    </tr>`;
  }).join('');
}
...
async function playCircleOfFourths() {
  ...
  const sequence = MinorSeventhFlatNineChordService.circleOfFourths;
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    cf.currentChord.textContent = `${entry.name}m7♭9`;
...
const jazzExercise = setupProgressionPicker('jz', InversionService.minorSeventhFlatNineJazzProgressions, midi);
const gospelExercise = setupProgressionPicker('gp', InversionService.minorSeventhFlatNineGospelProgressions, midi);
